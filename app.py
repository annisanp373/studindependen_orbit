...
def main():
    ...
    if st.sidebar.button("🏠 Home"):
        home()
    if st.sidebar.button("🔍 Scraping Data"):
        scraping()
    if st.sidebar.button("⚙️ Preprocessing & Labeling"):
        preprocessing()
    if st.sidebar.button("📊 Visualisasi Sentimen"):
        visualisasi()
    if st.sidebar.button("🤖 Modeling SVM"):
        modeling()
    if st.sidebar.button("🔮 Prediksi Sentimen"):
        prediksi_sentimen()
